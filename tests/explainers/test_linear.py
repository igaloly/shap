...
def test_tied_pair():
    ...
    X = np.ones((1,3))
    explainer = shap031.LinearExplainer((beta, 0), (mu, Sigma), feature_dependence="correlation")
...
def test_sklearn_linear():
    ...
    explainer = shap031.LinearExplainer(model, X)
...
def test_perfect_colinear():
    ...
    model.fit(X, y)
    explainer = shap031.LinearExplainer(model, X, feature_dependence="correlation")
